...
- (void)viewDidLoad
{
    ...
    if (self.chatRoom){
        ...
        self.navigationItem.rightBarButtonItem=[[UIBarButtonItem alloc]initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(onButtonInvite:)] ;
    }
    
     __weak __typeof__(self) weakSelf = self;
    if (!_tableView){
        CGRect rect=CGRectMake(0.0f, 0.0f, 320.0f, self.view.frame.size.height-SendMessageViewHeight);
        _tableView=[[ChatTableView alloc]initWithFrame:rect];
        _tableView.dataSource=self;
        _tableView.delegate=self;
        [self.view addSubview:_tableView];
        [self.tableView registerClass:[ChatTableViewCell class] forCellReuseIdentifier:ChatTableViewCellIdentifier];
        self.tableView.CloseKeyBoard = ^(){
    ...
    if (!_sendMessageView){
        ...
        _sendMessageText=[[UITextView alloc]initWithFrame:CGRectMake(0, Height(1), Width(240), Height(44))];
        _sendMessageText.font=[UIFont systemFontOfSize:17.0f];
        [_sendMessageView addSubview:_sendMessageText];
        _sendMessageButton=[[UIButton alloc]initWithFrame:CGRectMake(Width(240), Height(1), MSW-Width(240), Height(44))];
        [_sendMessageText becomeFirstResponder];
        _sendMessageButton.backgroundColor = [UIColor blueColor];
...
-(IBAction)onButtonInvite:(id)sender{
    InviteMemberToChatRoomViewController* inviteViewController=[[InviteMemberToChatRoomViewController alloc]initWithStyle:UITableViewStyleGrouped] ;
