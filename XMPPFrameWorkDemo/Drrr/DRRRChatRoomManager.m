            ...
            for (NSXMLElement* element in element_query.children) {
                if ([element.name isEqualToString:@"identity"]){
                    ...
                    for (NSXMLElement* element_field in element.children) {
                        ...
                        DRRRChatRoomInfoField* field=[[DRRRChatRoomInfoField alloc]initWithVar:var label:label value:value] ;
                        _fields[var]=field;
                    }
                }
            }
...
+(id)allocWithZone:(struct _NSZone *)zone{
    return [self sharedChatRoomManager];
}
...
-(DRRRChatRoom*)chatRoom:(NSString *)roomJid{
    ...
    if (!chatRoom){
        chatRoom=[[DRRRChatRoom alloc]initWithChatRoomJid:roomJid];
...
-(DRRRChatRoomMember*)member:(NSString *)jid
    ...
    if (!chatRoom){
        chatRoom=[[DRRRChatRoom alloc]initWithChatRoomJid:chatRoomJid] ;
        _chatRooms[chatRoomJid]=chatRoom;
    }
    DRRRChatRoomMember* member=chatRoom.memberList[jid];
    if (!member){
        member=[[DRRRChatRoomMember alloc]init];
...
-(void)xmppStream:(XMPPStream *)sender didReceivePresence:(XMPPPresence *)presence{
    if ([presence isChatRoomPresence]){
         ...
         */
        DRRRChatRoomMember* member=[[DRRRChatRoomMember alloc]initWithPresence:presence];
...
-(BOOL)xmppStream:(XMPPStream *)sender didReceiveIQ:(XMPPIQ *)iq{
    ...
    ///房间信息查询
        ...
        DRRRChatRoom* chatRoom=[self chatRoom:chatRoomJid];
        DRRRChatRoomInfo* chatRoomInfo=[[DRRRChatRoomInfo alloc]initWithIq:iq];
